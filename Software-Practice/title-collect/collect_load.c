...
void collect()
{
    ...
    if((d_shmid=shmget((key_t)D_KEY_NUM, dom_sz, IPC_CREAT|0666)) == -1) {
         perror("shmid failed");
         exit(1);
    }
    if((dom=(GET *)shmat(d_shmid, (void *)0, 0)) == (GET *)-1) {
        perror("shmat failed");
        exit(1);
    }
    if((i_shmid=shmget((key_t)I_KEY_NUM, sizeof(int), IPC_CREAT|0666)) == -1) {
         perror("shmid failed");
         exit(1);
    }
    if((pc=(int *)shmat(i_shmid, (void *)0, 0)) == (int *)-1) {
        perror("shmat failed");
        exit(1);
    }
    if((s_shmid=shmget((key_t)S_KEY_NUM, sizeof(int), IPC_CREAT|0666)) == -1) {
         perror("shmid failed");
         exit(1);
    }
    if((stat_cnt=(int *)shmat(s_shmid, (void *)0, 0)) == (int *)-1) {
        perror("shmat failed");
        exit(1);
    }
    ...
    while(1){
      //init
        ...
        if(strstr(btok[load_cnt],option[0])!=NULL || strstr(btok[load_cnt],option[1])!=NULL ){
            ...
            if(pid==0){
                sign=0;
                if((stin=open(ttok[0],O_RDONLY))==-1){ printf("nimi\n");
                  sleep(10000);
                }
