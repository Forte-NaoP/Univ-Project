...
void do_wget(char *dmn/*char *t_buff,char *i2s, int order, char *ttok, char *dtok, char *result, char *title, char *btok*/)
{
    pid_t pid;
    int2str(i2s,order);
    int i,j,k;
    ...
    int len;
    if((pid=fork())==0){
        execlp("wget", "wget","-q", "--no-check-certificate","-O", i2s, dmn,(char *)0);
    }
    else{
        wait(NULL);
...
void load(int *sign, int *stin, char *str)
{
    pid_t pid;
    int i,j,k;
    ...
    while(1){
      //init
        ...
        if(k==0){
            (*pc)++;
            order=(*pc);
            if(strstr(btok[0],option[0])!=NULL || strstr(btok[0],option[1])!=NULL ){
                do_wget(btok[0]);
                
            }//wget routine end
            else if(strstr(btok[0],option[2])!=NULL){ //print routine
                print(btok[0]);

            }
            else if(strstr(btok[0],option[3])!=NULL){ //stat routine
                stat();

            }
            else if(strstr(btok[0],option[4])!=NULL){ //load routine
                load(&sign, &stin, btok[0]);
                //exit(0);
            }
            else if(strstr(btok[0],option[5])!=NULL){ //quit routine
                quit();
            }
            else{ //input error
                //do nothing
            ...
            for(i=0;i<=k;i++){
                ...
                if(pid==0){
                    (*pc)++;
                    order=(*pc);
                    if(strstr(btok[i],option[0])!=NULL || strstr(btok[i],option[1])!=NULL ){
                        do_wget(btok[i]);
                        exit(0);
                    }//wget routine end
                    else if(strstr(btok[i],option[2])!=NULL){ //print routine
                        print(btok[i]);
                        exit(0);
                    }
                    else if(strstr(btok[i],option[3])!=NULL){ //stat routine
                        stat();
                        exit(0);
                    }
                    else if(strstr(btok[i],option[4])!=NULL){ //load routine
                        load(&sign, &stin, btok[i]);
                        //exit(0);
                    }
                    else if(strstr(btok[i],option[5])!=NULL){ //quit routine
                        quit();
                    }
                    else{ //input error
                        //do nothing
